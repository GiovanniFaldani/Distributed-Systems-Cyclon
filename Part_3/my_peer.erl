...
-import(nega_byzantine, [convalidate_view/2]).

-export([peer_execution/13]).
-export([peer_init/2]).
...
peer_execution(Server, PeerList, ViewSize, L, PermaList, IsVerbose, Turn, TurnDuration, TotalTurns, IsDone, ViewsConvalidated, RepList, ReqList) ->
    receive
        {turn, OldQ} ->
            if
                IsVerbose ->
            ...
            peer_execution(Server, NewPeerList, ViewSize, L, PermaList, IsVerbose, Turn + 1, TurnDuration, TotalTurns, false, ViewsConvalidated, RepList, ListForQ);

        {req_view, ListForQ, PID} ->
            if
                IsVerbose ->
        ...
        {rep_view, ListFromQ} ->
            ...
            RandomId = element(1, nth(1, shuffle_list(PeerList))),
            RandomId ! {req_val, ListFromQ, self()},

            peer_execution(Server, PeerList, ViewSize, L, PermaList, IsVerbose, Turn, TurnDuration, TotalTurns, true, ViewsConvalidated, ListFromQ, ReqList);


        {req_val, View, Pid} ->
            {Output, NewViewsConvalidated} = convalidate_view(View, ViewsConvalidated),
            Pid ! {rep_val, Output},
            peer_execution(Server, PeerList, ViewSize, L, PermaList, IsVerbose, Turn, TurnDuration, TotalTurns, IsDone, NewViewsConvalidated, RepList, ReqList);
        ...
        {rep_val, Output} ->
            ...
            peer_execution(Server, NewPeerList, ViewSize, L, PermaList, IsVerbose, Turn, TurnDuration, TotalTurns, IsDone, ViewsConvalidated, RepList, ReqList);
